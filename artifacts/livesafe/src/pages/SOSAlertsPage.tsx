import React from 'react'
import { useState } from 'react'
import { Bell, MapPin, CheckCircle, Clock, User, AlertTriangle, RefreshCw } from 'lucide-react'
import AppLayout from '@/components/layout/AppLayout'
import { useApi } from '@/app/hooks/useApi'
import { api } from '@/app/services/api'
import type { SOSAlert, SOSStatus } from '@/types'

const STATUS_CONFIG: Record<SOSStatus, { label: string; color: string; bg: string }> = {
  active:       { label: 'Active',       color: '#ef4444', bg: 'rgba(239,68,68,0.12)' },
  acknowledged: { label: 'Acknowledged', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' },
  resolved:     { label: 'Resolved',     color: '#22c55e', bg: 'rgba(34,197,94,0.12)' },
}

function formatTime(iso: string) {
  const d = new Date(iso)
  const now = Date.now()
  const diff = now - d.getTime()
  const mins = Math.floor(diff / 60000)
  if (mins < 1) return 'Just now'
  if (mins < 60) return `${mins}m ago`
  const hrs = Math.floor(mins / 60)
  if (hrs < 24) return `${hrs}h ago`
  return d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })
}

function AlertCard({ alert, onAck }: { alert: SOSAlert; onAck: (id: string) => Promise<void>; key?: React.Key }) {
  const cfg = STATUS_CONFIG[alert.status]
  const [loading, setLoading] = useState(false)

  const handleAck = async () => {
    setLoading(true)
    try {
      await onAck(alert.id)
    } finally {
      setLoading(false)
    }
  }

  return (
    <div className={`alert-card ${alert.status === 'active' ? 'pulsing' : ''}`}>
      <div className="alert-header">
        <div className="alert-user">
          <div className="avatar">{(alert.user_name ?? '?').charAt(0)}</div>
          <div>
            <div className="alert-name">{alert.user_name ?? 'Unknown User'}</div>
            <div className="alert-time"><Clock size={11} /> {formatTime(alert.created_at)}</div>
          </div>
        </div>
        <div className="alert-status" style={{ background: cfg.bg, color: cfg.color }}>
          {cfg.label}
        </div>
      </div>

      <div className="alert-location">
        <MapPin size={13} color="#94a3b8" />
        <span>{alert.latitude.toFixed(4)}, {alert.longitude.toFixed(4)}</span>
        <a
          href={`https://www.google.com/maps/search/?api=1&query=${alert.latitude},${alert.longitude}`}
          target="_blank"
          rel="noopener noreferrer"
          className="map-link"
        >
          View on Map →
        </a>
      </div>

      {alert.assigned_officer && (
        <div className="alert-officer">
          <User size={12} color="#818cf8" />
          <span>Assigned to: <strong>{alert.assigned_officer}</strong></span>
        </div>
      )}

      {alert.response_time && (
        <div className="alert-officer">
          <CheckCircle size={12} color="#22c55e" />
          <span>Response time: <strong>{Math.round(alert.response_time / 60)}m {alert.response_time % 60}s</strong></span>
        </div>
      )}

      {alert.status === 'active' && (
        <button className="btn btn-primary ack-btn" onClick={handleAck} disabled={loading}>
          {loading ? 'Acknowledging…' : '✓ Acknowledge Alert'}
        </button>
      )}
    </div>
  )
}

export default function SOSAlertsPage() {
  const { data: alerts, isLoading, error, refetch } = useApi((sig) => api.getSOSAlerts(sig))
  const [filter, setFilter] = useState<SOSStatus | 'all'>('all')
  const [localAlerts, setLocalAlerts] = useState<SOSAlert[] | null>(null)

  const displayAlerts = localAlerts ?? alerts ?? []
  const filtered = filter === 'all' ? displayAlerts : displayAlerts.filter((a: SOSAlert) => a.status === filter)
  const activeCount = displayAlerts.filter((a: SOSAlert) => a.status === 'active').length

  const handleAck = async (id: string) => {
    const updated = await api.acknowledgeSOSAlert(id)
    setLocalAlerts((prev: SOSAlert[] | null) =>
      (prev ?? displayAlerts).map((a: SOSAlert) => (a.id === id ? updated : a))
    )
  }

  return (
    <AppLayout title="SOS Alerts" subtitle="Active emergency alerts — respond within 3 minutes">
      <div className="sos-page">
        {/* Header stats */}
        <div className="sos-header-stats">
          <div className="sos-stat active">
            <Bell size={20} color="#ef4444" />
            <div>
              <div className="sos-stat-val">{activeCount}</div>
              <div className="sos-stat-lbl">Active Alerts</div>
            </div>
          </div>
          <div className="sos-stat">
            <CheckCircle size={20} color="#22c55e" />
            <div>
              <div className="sos-stat-val">{displayAlerts.filter((a: SOSAlert) => a.status === 'resolved').length}</div>
              <div className="sos-stat-lbl">Resolved Today</div>
            </div>
          </div>
          <div className="sos-stat">
            <Clock size={20} color="#f59e0b" />
            <div>
              <div className="sos-stat-val">8.4m</div>
              <div className="sos-stat-lbl">Avg Response</div>
            </div>
          </div>
        </div>

        {/* Filters */}
        <div className="sos-filters">
          {(['all', 'active', 'acknowledged', 'resolved'] as const).map((f: 'all' | 'active' | 'acknowledged' | 'resolved') => (
            <button
              key={f}
              className={`filter-btn ${filter === f ? 'active' : ''}`}
              onClick={() => setFilter(f)}
            >
              {f.charAt(0).toUpperCase() + f.slice(1)}
              {f === 'active' && activeCount > 0 && (
                <span className="filter-badge">{activeCount}</span>
              )}
            </button>
          ))}
          <button className="btn btn-ghost refresh-btn" onClick={refetch}>
            <RefreshCw size={14} />
            Refresh
          </button>
        </div>

        {/* List */}
        {isLoading && (
          <div className="loading-state">
            <div className="spinner" />
            <span>Loading alerts…</span>
          </div>
        )}
        {error && (
          <div className="error-state">
            <AlertTriangle size={24} color="#ef4444" />
            <span>Failed to load alerts. Please refresh.</span>
          </div>
        )}
        {!isLoading && !error && filtered.length === 0 && (
          <div className="empty-state">
            <Bell size={40} color="#334155" />
            <p>No {filter !== 'all' ? filter : ''} alerts found.</p>
          </div>
        )}
        <div className="alerts-grid">
          {filtered.map((a: SOSAlert) => (
            <AlertCard key={a.id} alert={a} onAck={handleAck} />
          ))}
        </div>
      </div>

      <style>{`
        .sos-page { display: flex; flex-direction: column; gap: 1.25rem; }
        .sos-header-stats {
          display: flex;
          gap: 1rem;
          flex-wrap: wrap;
        }
        .sos-stat {
          background: #1e293b;
          border: 1px solid rgba(255,255,255,0.06);
          border-radius: 10px;
          padding: 1rem 1.25rem;
          display: flex;
          align-items: center;
          gap: 0.85rem;
          flex: 1;
          min-width: 140px;
        }
        .sos-stat.active { border-color: rgba(239,68,68,0.25); background: rgba(239,68,68,0.06); }
        .sos-stat-val { font-size: 1.6rem; font-weight: 800; color: #f1f5f9; line-height: 1; }
        .sos-stat-lbl { font-size: 0.73rem; color: #64748b; margin-top: 2px; }
        .sos-filters {
          display: flex;
          gap: 0.5rem;
          flex-wrap: wrap;
          align-items: center;
        }
        .filter-btn {
          display: flex;
          align-items: center;
          gap: 6px;
          padding: 0.4rem 0.9rem;
          border-radius: 7px;
          background: rgba(30,41,59,0.6);
          border: 1px solid rgba(255,255,255,0.07);
          color: #94a3b8;
          cursor: pointer;
          font-size: 0.82rem;
          font-weight: 500;
          transition: all 0.15s;
        }
        .filter-btn:hover { background: rgba(255,255,255,0.05); color: #f1f5f9; }
        .filter-btn.active { background: rgba(99,102,241,0.15); border-color: rgba(129,140,248,0.4); color: #818cf8; font-weight: 600; }
        .filter-badge {
          background: #ef4444;
          color: #fff;
          border-radius: 9999px;
          padding: 0 5px;
          font-size: 0.68rem;
          font-weight: 700;
          min-width: 16px;
          text-align: center;
        }
        .refresh-btn { margin-left: auto; }
        .alerts-grid {
          display: grid;
          grid-template-columns: repeat(auto-fill, minmax(320px, 1fr));
          gap: 1rem;
        }
        .alert-card {
          background: #1e293b;
          border: 1px solid rgba(255,255,255,0.06);
          border-radius: 12px;
          padding: 1.1rem;
          display: flex;
          flex-direction: column;
          gap: 0.75rem;
          transition: border-color 0.2s;
        }
        .alert-card.pulsing {
          border-color: rgba(239,68,68,0.4);
          animation: alert-pulse 2.5s ease-in-out infinite;
        }
        @keyframes alert-pulse {
          0%, 100% { box-shadow: 0 0 0 0 rgba(239,68,68,0); }
          50%       { box-shadow: 0 0 0 6px rgba(239,68,68,0.1); }
        }
        .alert-header { display: flex; justify-content: space-between; align-items: flex-start; }
        .alert-user { display: flex; align-items: center; gap: 0.65rem; }
        .avatar {
          width: 36px;
          height: 36px;
          border-radius: 50%;
          background: rgba(99,102,241,0.2);
          border: 1.5px solid rgba(129,140,248,0.3);
          display: flex;
          align-items: center;
          justify-content: center;
          font-weight: 700;
          font-size: 0.9rem;
          color: #818cf8;
          flex-shrink: 0;
        }
        .alert-name { font-size: 0.88rem; font-weight: 600; color: #f1f5f9; }
        .alert-time { display: flex; align-items: center; gap: 3px; font-size: 0.72rem; color: #64748b; margin-top: 1px; }
        .alert-status {
          padding: 2px 10px;
          border-radius: 9999px;
          font-size: 0.7rem;
          font-weight: 700;
          text-transform: uppercase;
          letter-spacing: 0.05em;
          flex-shrink: 0;
        }
        .alert-location {
          display: flex;
          align-items: center;
          gap: 6px;
          font-size: 0.78rem;
          color: #94a3b8;
          flex-wrap: wrap;
        }
        .map-link {
          color: #818cf8;
          text-decoration: none;
          font-weight: 600;
          margin-left: auto;
          font-size: 0.75rem;
        }
        .map-link:hover { text-decoration: underline; }
        .alert-officer {
          display: flex;
          align-items: center;
          gap: 5px;
          font-size: 0.78rem;
          color: #94a3b8;
        }
        .alert-officer strong { color: #f1f5f9; }
        .ack-btn { align-self: flex-start; font-size: 0.82rem; padding: 0.45rem 1rem; }
        .loading-state, .error-state, .empty-state {
          display: flex;
          flex-direction: column;
          align-items: center;
          justify-content: center;
          gap: 0.75rem;
          padding: 3rem;
          color: #64748b;
          font-size: 0.9rem;
        }
        .empty-state p { margin: 0; }
      `}</style>
    </AppLayout>
  )
}
